		...
		foreach($allmoots as $mt){
			...
			if($log_user == TRUE) {
				if($log_user_id==$fan_id || $log_user_role=='editor' || $log_user_role=='administrator'){
					...
					';
					$edit_btn = '
						<a href="javascript:;" class="text-muted mr10" onclick="editmoot'.$moot_id.'();"><i class="ti-pencil mr5"></i>Edit</a>
						<script type="text/javascript">
							function editmoot'.$moot_id.'(){
								var edittext = document.getElementById("edit_text'.$moot_id.'").value;
								document.getElementById("moot-edit'.$moot_id.'").innerHTML = "<input type=\"text\" value=+edittext+ class=\"form-control\" />";
							}
						</script>
					';
				} else {$del_btn='';$edit_btn='';}
			} else {$del_btn='';$edit_btn='';}
			
			//display moots
			if($page_active == 'club'){ //for country clubs
				if($status==1){
					if($log_user_country == $club_country){
						$moot_list .= '
							<div id="moot-del-msg'.$moot_id.'" class="media p15">
								...
								<div class="comment">
									...
									<p>
										<input type="hidden" id="edit_text'.$moot_id.'" value="'.$moot_detail.'" />
										<div id="moot-edit'.$moot_id.'">'.$moot_detail.'</div>
									</p>
									<p class="small">
				...
				if($privacy == 0 && $status == 1){
					//disable comment if not public
					if($log_user_country != $club_country){$com_form = '';}
