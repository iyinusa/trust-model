...
class Profession extends CI_Controller {
	...
	public function index() 
	{
		...
		$data['title'] = 'Manage Professions | '.app_name;
		$data['page_active'] = 'profession';
	...
	public function add(){
		...
		if($get_id != ''){
			$gq = $this->m_profession->query_profession_id($get_id);
			foreach($gq as $item){
				$data['e_id'] = $item->id;
		...
		$this->form_validation->set_rules('name','Profession Name','trim|required|xss_clean');
		...
		else
		{
			...
			if($_POST){
				...
				//check for existence
				if(($this->m_profession->check_by_name($name) > 0) && $profession_id == ''){
					$data['err_msg'] = '<div class="alert alert-info"><h5><i class="fa fa-flash fa-2x"></i> Profession already in database</h5></div>';
				} else {
					//check for update
					if($profession_id != ''){
						$upd_data = array(
						...
						if($this->m_profession->update_profession($profession_id, $upd_data) > 0){
						...
						if($this->m_profession->reg_insert($reg_data) > 0){
	...
	public function delete(){
		...
		if(isset($_POST['cancel'])){
			...
			if($del_id != ''){
				if($this->m_profession->delete_profession($del_id) > 0){
					redirect(base_url('profession/add'), 'refresh');
				} else {
